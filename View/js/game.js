//Settings
var mindPathsBackEndUrl = "/semantics-backend/";
...
function startOver() {
	...
	$("#wordImg" + steps).attr("src", "/semantics/images/endrow.png");
...
function getRelatedWords() {
	...
	request.open('GET', mindPathsBackEndUrl + 'links/' + activeWordId, true);
	request.send();
}
...
function setWordBarItem(index, checked, word) {
	...
	if (checked) {
		$(image).attr("src", "/semantics/images/activeRow.png");
		$(label).css({ color: '#2B3126' });
	}
	else {
		$(image).attr("src", "/semantics/images/empty_row.png");
...
function setWordBarEndItem(index) {
	var image = "#wordImg" + index;
	$(image).attr("src", "/semantics/images/endrow.png");
...
function toggleResetButton(state){
	...
	ChangeImage(document.getElementById('imgResetButton'), '/semantics/images/reset.png');
	resetState = state;
}